...
call. Each record becomes a `structured` document (see `openodke.loaders.records`),
...
from openodke.loaders.base import import_extra, read_text
from openodke.loaders.records import parse_path, record_document, resolve_path
...
class ParquetLoader:
    """Parquet, one document per row. Needs `pip install "openodke[parquet]"`.
