...
from odke.ontology.load import load_dict, report_problems
...
def ontology_from_neo4j(
    ...
    *,
    ...
    from odke.sinks.neo4j import _connect
...
def _reflect(driver: Any, database: str | None) -> tuple[dict[str, Any], list[str]]:
    from odke.sinks.neo4j import _rows
...
class _Reflection:
    def __init__(self, read: Read) -> None:
        from odke.sinks.neo4j import _ENTITY_FIELDS, _PROVENANCE, CLAIM_LABEL, ENTITY_LABEL
    ...
    def edges(self) -> dict[str, dict[str, Any]]:
        from odke.sinks.neo4j import _ident
