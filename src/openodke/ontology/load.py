...
imports `openodke.ontology` at module level: the model classes arrive as `cls`, so
...
def load_yaml(cls: type[OntologyT], source: Source, *, strict: bool) -> OntologyT:
    try:
        ...
        raise ImportError('PyYAML is not installed. Run: pip install "openodke[yaml]"') from exc
    text, where = _read(source)
    try:
