...
from odke.eval.cost import StageCost
from odke.eval.extraction import evaluate_extraction, match_extraction
from odke.eval.formats import GoldFact, GroundingLabel
from odke.eval.grounding import DROPPED, grounding_ablation
from odke.eval.report import Metric, StageReport
from odke.ontology import Ontology
from odke.pipeline import Pipeline
from odke.types import Chunk, Document, Fact, GroundingVerdict

if TYPE_CHECKING:
    from odke.run.config import RunConfig
...
def run_ablation(config: RunConfig, gold: Sequence[GoldFact]) -> StageReport:
    ...
    # Imported here: `odke.run` imports `odke.eval.cost`, and so this package.
    from odke.run.build import build
    from odke.run.execute import register_documents
